...
/** Build a fully-typed pg.QueryResult from just the rows array. */
...
const mockPoolQuery = pool.query as Mock<(...args: unknown[]) => Promise<QueryResult>>;
...
describe('Projects API', () => {
  ...
  describe('GET /api/projects', () => {
    it('returns array with ice_score computed field', async () => {
      ...
      mockPoolQuery.mockResolvedValueOnce(mockQueryResult(mockProjects));
    ...
    it('returns projects sorted by ice_score descending by default', async () => {
      ...
      const lastCall = mockPoolQuery.mock.calls.pop();
  ...
  describe('POST /api/projects', () => {
    it('creates project without owner_id (optional)', async () => {
      ...
      mockPoolQuery
        .mockResolvedValueOnce(mockQueryResult([mockProject]));
    ...
    it('creates project with valid data including optional owner_id', async () => {
      ...
      mockPoolQuery
        ...
        .mockResolvedValueOnce(mockQueryResult([mockProject]))
        // Get user info
        .mockResolvedValueOnce(mockQueryResult([{ id: ownerId, name: 'Test Owner', email: 'owner@example.com' }]));
  ...
  describe('GET /api/projects/:id', () => {
    it('returns project with ice_score computed', async () => {
      ...
      mockPoolQuery.mockResolvedValueOnce(mockQueryResult([mockProject]));
  ...
  describe('PATCH /api/projects/:id', () => {
    it('updates ICE properties', async () => {
      ...
      mockPoolQuery
        // Check existing
        .mockResolvedValueOnce(mockQueryResult([existingProject]))
        // Update
        .mockResolvedValueOnce(mockQueryResult([]))
        // Re-query
        .mockResolvedValueOnce(mockQueryResult([updatedProject]));
  ...
  describe('DELETE /api/projects/:id', () => {
    it('deletes project and removes references', async () => {
      mockPoolQuery
        // Access check
        .mockResolvedValueOnce(mockQueryResult([{ id: 'project-123' }]))
